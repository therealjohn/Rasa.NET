...
This reference describes the bounded P3 implementation for maintainers. It uses
the approved community historical tables, not a claim of exact client 1.16.5.0
balance or native-client acceptance.
...
| Other catalogue abilities | None added by P3 | Existing training and drawer mappings remain available; execution fails explicitly. P12 remains separate. |
...
- Rank 1 rolls inclusively from 180 through 240. Rank 2 rolls inclusively from
  ...
  percentage without applying it to Lightning. P3 explicitly adopts it as a
...
```powershell
dotnet test src\Rasa.Test\Rasa.Test.csproj --configuration Release --no-build --no-restore --filter "FullyQualifiedName~LightningRecoveryTests|FullyQualifiedName~LightningArcTests|FullyQualifiedName~AbilityTests|FullyQualifiedName~AbilityTrayTests|FullyQualifiedName~HudStateTests|FullyQualifiedName~GameplayTravelTests"
```

The fixtures exercise manager entry points, outgoing serialization, deterministic
time/random seams, real migrated SQLite transactions, rollback and reopening.
The database compatibility suite also compares both provider models with their
snapshots and generates MySQL migration SQL. The opt-in live MySQL P3 cases
exercise the additive selection migration, training/tray reopen behavior and
shared-transaction rollback; see [MySQL persistence checks](world-testing.md#mysql-persistence-checks).
...
remain external acceptance work. These changes do not complete all P3 work,
native 1.16.5.0 acceptance or the P12 class/tier catalogue.
