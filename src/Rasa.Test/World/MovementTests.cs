...
namespace Rasa.Test.World
{
    ...
    public class MovementTests
    {
        ...
        [TestMethod]
        public void MovementUpdatesVisibilityBeforeBroadcastingThePosition()
        {
            using var world = new WorldTestContext();
            ...
            Assert.IsTrue(moving.HandleMovement(new Movement(new Vector3(200, 0, 0), Vector2.Zero)));

            var oldPackets = WorldTestContext.Drain(oldNeighbor);
            Assert.IsFalse(oldPackets.Any(p => p.Message is MoveObjectMessage));
            Assert.IsTrue(oldPackets.Any(p => p.Message is CallMethodMessage c &&
                c.Packet is DestroyPhysicalEntityPacket d && d.EntityId == moving.Player.EntityId));
            var newPackets = WorldTestContext.Drain(newNeighbor);
            var introduced = newPackets.FindIndex(p => p.Message is CallMethodMessage c &&
                c.Packet is CreatePhysicalEntityPacket e && e.EntityId == moving.Player.EntityId);
            var moved = newPackets.FindIndex(p => p.Message is MoveObjectMessage);
            Assert.IsTrue(introduced >= 0 && moved > introduced);
            Assert.AreEqual(CellManager.Instance.GetCellSeed(moving.Player.Position), moving.Player.Cells[2, 2]);
        }
